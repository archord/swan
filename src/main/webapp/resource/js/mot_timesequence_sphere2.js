...
(function($) {
  ...
  Gwac.prototype = {
    ...
    parseData: function(reqData) {
      ...
      for (var i = 0; i < gwac.motObj.length; i++) {
        gwac.mot[i] = [];
      }
      ...
      $.each(gwac.motObj, function(i, item) {
        ...
        $.each(item.mov_detail, function(j, item2) {
          item2.dateObj = new Date(item2.date_ut);
        });

        item.mov_detail.sort(function(a, b) {
          return (a.dateObj > b.dateObj) ? 1 : ((a.dateObj < b.dateObj) ? -1 : 0);
        });
      });
      this.genLabelPoint();
    },
...
//                svg.append("path").datum(gwac.singleLineStringExample.data).attr("class", gwac.primemeridian.class).attr("d", path);
      ...
      gwac.zoomBounds(gwac.ot1Data.data);
