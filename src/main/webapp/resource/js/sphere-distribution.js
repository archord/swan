...
(function($) {
  ...
  Gwac.prototype = {
    ...
    initView: {data: {type: "MultiPoint", coordinates: [[10, 10], [80, 80]]}, class: "ot1", radius: 1},
    ...
    draw: function() {
      ...
      var zoom = d3.geo.zoom().projection(projection)
              ...
              .on("zoom", function() {
                ...
                if (gwac.reqData.objs !== undefined)
                  for (var i = 0; i < gwac.reqData.objs.length; i++) {
                    var tstar = gwac.reqData.objs[i];
                    var tRadius = (tstar.mag*(-1) + 17)/3;
                    if(tRadius<1){
                      tRadius = 1;
                    }else if(tRadius>4){
                      tRadius = 4;
                    }
                    var tnode = svg.append("path").datum({type: "Point", coordinates: [tstar.ra, tstar.dec]}).attr("class", gwac.ot2Data.class).attr("d", path.pointRadius(tRadius));
                    tnode.append("title").text(tstar.name + "(" + tstar.ra + "," + tstar.dec + ")");
      ...
      gwac.zoomBounds(projection, zoom, path, gwac.initView.data);
