...
//        }));
  ...
  function reConstructParameter(data) {
    ...
    parms = parms + '&ot2qp.length=' + data.length;
    parms = parms + '&ot2qp.start=' + data.start;
    console.log(parms);
    return parms;
  }
  function reConstructParameter2(data) {
    var parms = 'draw=' + data.draw;
    parms = parms + '&start=' + data.start;
    parms = parms + '&length=' + data.length;
    parms = parms + '&search.value=' + data.search.value;
    parms = parms + '&search.regex=' + data.search.regex;
    for (var i = 0; i < data.columns.length; i++) {
      var column = data.columns[i];
      parms = parms + '&columns[' + i + '].data=' + column.data;
      parms = parms + '&columns[' + i + '].name=' + column.name;
      parms = parms + '&columns[' + i + '].searchable=' + column.searchable;
      parms = parms + '&columns[' + i + '].orderable=' + column.orderable;
    }
    for (var i = 0; i < data.order.length; i++) {
      var order = data.order[i];
      parms = parms + '&order[' + i + '].column=' + order.column;
      parms = parms + '&order[' + i + '].dir=' + order.dir;
    }
    console.log(parms);
    return parms;
  }
